...
const AnimatedIcon = ({ name, focused }: { name: string; focused: boolean }) => {
  ...
  return (
    <Animated.View style={animatedStyle}>
      <Icon name={name} size={20} color={focused ? Theme.colors.Primary : Theme.colors.GrayLight} />
    </Animated.View>
  );
};
  ...
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        ...
        tabBarLabel: ({ focused }) => {
          return (
            <Text style={{ fontSize: 10, fontWeight:"bold", color: focused ? Theme.colors.Primary : Theme.colors.GrayLight }}>
              {route.name}
            </Text>
          );
        },
